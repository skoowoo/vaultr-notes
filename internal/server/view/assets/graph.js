  ...
  function graphCtrl() {
    return Object.assign(drawerCtrl(), {
      ...
      _applyFocus(node) {
        ...
        this.cy.animate({
          center: { eles: node },
          zoom: Math.max(this.cy.zoom(), 0.6),
        }, { duration: 300 });
      ...
      _tagPaletteColor(tag) {
        ...
        var palette = ['#3b82f6','#ec4899','#8b5cf6','#34d399','#f59e0b','#cc785c','#06b6d4','#f97316'];
        var h = 0;
      ...
      _tagBorderBright(tag) {
        var hex = this._tagPaletteColor(tag).replace(/^#/, '');
        var r = parseInt(hex.slice(0,2), 16);
        var g = parseInt(hex.slice(2,4), 16);
        var b = parseInt(hex.slice(4,6), 16);
      ...
      _renderGraph(data) {
        ...
        function nodeSize(id) {
          ...
          return Math.round(Math.min(80, 16 + Math.log2(deg + 1) * 10));
        }
        ...
        (data.edges || []).forEach(function(e) {
          ...
          var edgeColor = /^#[0-9a-f]{6}$/i.test(borderHex)
            ? self._hexToRgba(borderHex, 0.38)
            : edgeFallback;
          elements.push({ data: { source: e.source, target: e.target, edgeColor: edgeColor } });
        });

        var nc = (data.nodes || []).length;
        // Small graphs get higher-quality proof layout; huge graphs trade quality for speed.
        var layoutQuality  = nc <= 60 ? 'proof' : 'default';
        var layoutNumIter  = nc <= 60 ? 1500 : nc <= 250 ? 2500 : 1800;
        var layoutRepulsion  = Math.max(4500,  nc * 150);
        var layoutEdgeLen    = Math.max(80, Math.min(260, 60 + nc * 2));
        // Gravity lower-bound raised to 0.10 to prevent sparse graphs from drifting apart.
        ...
        this.cy = cytoscape({
          ...
          style: [
            ...
            },
            {
              ...
              style: {
                'border-color': function(ele) { return self._tagBorderBright(ele.data('entityType')); },
                'border-width': 3.5,
                'shadow-blur': 18,
                'shadow-color': function(ele) { return self._tagBorderBright(ele.data('entityType')); },
                'shadow-opacity': 0.90,
                'shadow-offset-x': 0,
                'shadow-offset-y': 0,
                'z-index': 20,
